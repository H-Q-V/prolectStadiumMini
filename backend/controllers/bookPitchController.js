...
const bookPitchController = {
  bookPitch: async (req, res) => {
    try {
      ...
      const stadiums = await BookPitch.findOne({
        stadium: stadium,
      }).populate("stadium");
      const staiumOwnerID = stadiums.stadium.stadium_owner;
  ...
  getStadiumOwnerBookPitch: async () => {
    try {
    } catch (error) {
  ...
  payBookPitches: async (req, res) => {
    try {
      ...
      const bookings = await BookPitch.findOne({ user: userId }).populate(
        "stadium"
      );
